...
<section class="detail">
    ...
    <aside class="calendar">
        <div class="months">
            <a href="/news-and-events/events-calendar/<?php echo date("Y", mktime(0, 0, 0, $month-1, 1, $year)); ?>/<?php echo date("m", mktime(0, 0, 0, $month-1, 1, $year)); ?>"><span class="hidden">previous</span></a>
            <span class="current-month"><?php echo date("M Y", mktime(0, 0, 0, $month, 1, $year)); ?></span>
            <a href="/news-and-events/events-calendar/<?php echo date("Y", mktime(0, 0, 0, $month+1, 1, $year)); ?>/<?php echo date("m", mktime(0, 0, 0, $month+1, 1, $year)); ?>" class="next"><span class="hidden">next</span></a>
        </div>
        <table>
            <thead class="days-of-week">
            <tr>
                <td title="Sunday">S<span class="hidden">unday</span></td>
                <td title="Monday">M<span class="hidden">onday</span></td>
                <td title="Tuesday">T<span class="hidden">uesday</span></td>
                <td title="Wednesday">W<span class="hidden">ednesday</span></td>
                <td title="Thursday">T<span class="hidden">hursday</span></td>
                <td title="Friday">F<span class="hidden">riday</span></td>
                <td title="Saturday">S<span class="hidden">aturday</span></td>
            </tr>
            </thead>
...
$num = cal_days_in_month(CAL_GREGORIAN, $month, $year);
    for ($i = 1; $i <= $num; $i++) {
        ...
        $rowcount++;
        ?>
       <td<?php if ( $loop->have_posts() ) echo ' class="has-events"'; ?>><a title="<?php echo date("d-M-Y", mktime(0, 0, 0, $month, $i, $year)); ?>" href="/news-and-events/events-calendar/<?php echo date("Y", mktime(0, 0, 0, $month, $i, $year)); ?>/<?php echo date("m", mktime(0, 0, 0, $month, $i, $year)); ?>/<?php echo date("d", mktime(0, 0, 0, $month, $i, $year)); ?>">
...
?>
    ...
    <?php if($date[2]!='') : ?>
    <li>
        <h2 class="noevent">We have no events scheduled this month. Use the links below to search for forthcoming events</h2>
    </li>
    <?php else : ?>
    <li>
        <h2 class="noevent">We have no events scheduled this month. Use the links below to search for forthcoming events</h2>
...
$rearslide = false;
 while($rearslide == false && $y<12) {
    ...
    <div class="alt-dates">
        <?php if($x<12) : ?>
            <a class="right" href="/news-and-events/events-calendar/<?php echo date("Y", mktime(0, 0, 0, $foremonth, 1, $foreyear)); ?>/<?php echo date("m", mktime(0, 0, 0, $foremonth, 1, $foreyear)); ?>">Forthcoming Events</a>
        <?php endif;
        if($y<12) :?>
            <a href="/news-and-events/events-calendar/<?php echo date("Y", mktime(0, 0, 0, $rearmonth, 1, $rearyear)); ?>/<?php echo date("m", mktime(0, 0, 0, $rearmonth, 1, $rearyear)); ?>">Previous Events</a>
        <?php endif; ?>
    </div>
