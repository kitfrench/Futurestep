  ...
  <section class="core-content">
    <section class="overview">
      <nav class="index">
        <ul>
          <li<?php if($sector=='') echo ' class="selected" ';?>>
            <a href="<?php echo get_bloginfo('siteurl').'/working-with-us/';?>">All Sectors</a></li>
          <?php
          ...
          <?php if($sector==$term->slug) echo ' class="selected" ';?>><a
                href="<?php echo get_bloginfo('siteurl').'/working-with-us/'.$term->slug; ?>"><?php echo $term->name;
          ?></a>
          </li>
          <?php  endforeach; ?>
        </ul>
      ...
      <ul class="client-list">
        ...
        <?php
            $args = array( 'post_type' => 'case-studies', 'posts_per_page' => 0, 'category-sector' => $sector);
        ...
        <?= $poppableCls?>>
        <div>
        <a class="client-logo" title="<?= $post->post_name ?>" href="<?php the_permalink(); ?>">
          <?php
            echo clean_wp_width_height(get_the_post_thumbnail($post->ID, 'fullsize'));
