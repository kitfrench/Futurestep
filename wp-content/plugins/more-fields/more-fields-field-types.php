...
//		$f['wysiwyg']['actions'] = array('mf_the_editor' => array( '%value%', '%key%'));
		...
		*/		
		...
		$f['file-list']['html_item'] = "
                        <a style='padding-bottom: 10px;' class='thickbox' title='Add Media' href='media-upload.php?post_id=".$_GET['post']."&TB_iframe=1'>Upload a file</a>
		...
		jQuery('.file_list_update').bind('click', function(){
			...
			jQuery.post(url, data, function(response) {
				...
				jQuery.each(json['data'], function(i, file){
					...
					htmlth = htmlth + '<option value=\"' + file['guid'] + '\">' + file['post_title'] + ' (' + file['post_mime_type']  + ')</option>';				
				});
